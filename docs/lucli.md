...
### Docker Compose (LuCLI + Nginx)
Use the provided `docker-compose.yml` to build a LuCLI-powered Lucee server that proxies through Nginx and serves `www/index.cfm`:

```bash
# build LuCLI image (uses the checked-in lucli.jar) and start the stack
...
Details:
- `lucee` service builds from `docker/lucli/Dockerfile`, copies in your local `lucli.jar`, and runs `java -jar lucli.jar server run --port 8080 --webroot /var/www` (override with `docker compose build --build-arg LUCLI_JAR_URL=... lucee` if you need to auto-download)
- `www/` is bind-mounted into both containers so LuCLI serves the same files that Nginx exposes
- `nginx` reverse proxies CFML requests to `lucee:8080` (see `docker/nginx/default.conf`)
- Exposes the site on http://localhost:8080

Rebuild the Docker image (`docker compose build lucee`) whenever you swap in a different `lucli.jar`.
## 🚀 Usage Examples
